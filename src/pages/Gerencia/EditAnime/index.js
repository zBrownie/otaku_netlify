...
import { updateAnime } from "../../../services/firebase_actions";
...
export default function EditAnime() {
  ...
  const [pos, setpos] = useState(anime.pos);
  const [dia, setdia] = useState(anime.dia);
  const [title, settitle] = useState(anime.title);
  const [desc, setdesc] = useState(anime.desc);
  const [link, setlink] = useState(anime.link);
  const [link2, setlink2] = useState(anime.link2);
  const [imgUrl, setimgUrl] = useState(anime.imgUrl);
  const [streaming, setstreaming] = useState(anime.streaming);
  ...
  const handleRegisterAnime = async (e) => {
    ...
    await updateAnime(data).then((resp) => {
      history.push("/");
    ...
    <Container component="main" maxWidth="xs">
      ...
      <div className={classes.paper}>
        <div className={classes.avatar}>
          <img
            src={file == null ? anime.imgUrl : URL.createObjectURL(file)}
            alt="Preview Logo"
